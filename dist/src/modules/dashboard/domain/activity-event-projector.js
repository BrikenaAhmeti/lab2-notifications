...
exports.activityDomainEventTypes = void 0;
exports.projectActivityFromDomainEvent = projectActivityFromDomainEvent;
exports.activityDomainEventTypes = [
...
function projectActivityFromDomainEvent(input) {
    const data = input.data ?? {};
    const actorName = text(data, [
        'activityActorName',
        'actorName',
        'updatedByName',
        'createdByName',
...
function withOverrides(data, input) {
...
function normalizeActivityLink(link) {
    if (!link)
        return null;
...
function appointmentLabel(data) {
    return text(data, ['activityEntityLabel', 'appointmentLabel', 'entityLabel', 'serviceName'], `Appointment for ${patientName(data)}`);
}
function appointmentLink(data, fallback) {
    return linkFrom(data, ['activityEntityLink', 'entityLink', 'appointmentLink'], ['appointmentId', 'entityId'], '/appointments', fallback);
}
function labOrderLabel(data) {
    const id = optionalText(data, ['labOrderId', 'entityId']);
    return text(data, ['activityEntityLabel', 'labOrderLabel', 'entityLabel', 'testName'], id ? `Lab order ${id}` : `Lab results for ${patientName(data)}`);
}
function labOrderLink(data, fallback) {
    return linkFrom(data, ['activityEntityLink', 'entityLink', 'labOrderLink'], ['labOrderId', 'entityId'], '/lab/orders', fallback);
}
function billingLabel(data) {
    return text(data, ['activityEntityLabel', 'billingLabel', 'invoiceNumber', 'entityLabel'], `Billing for ${patientName(data)}`);
}
function billingLink(data, fallback) {
    return linkFrom(data, ['activityEntityLink', 'entityLink', 'billingLink'], ['billingId', 'invoiceId', 'entityId'], '/admin/billing', fallback);
}
function inventoryItemName(data) {
    return text(data, ['activityEntityLabel', 'itemName', 'inventoryItemName', 'entityLabel'], 'Inventory item');
}
function inventoryLink(data, fallback) {
    return linkFrom(data, ['activityEntityLink', 'entityLink', 'inventoryLink'], ['inventoryItemId', 'itemId', 'entityId'], '/admin/inventory/items', fallback);
}
function statusName(data, fallback) {
...
function linkFrom(data, linkKeys, idKeys, basePath, fallback) {
...
function text(data, keys, fallback) {
    return optionalText(data, keys) ?? fallback;
}
function optionalText(data, keys) {
